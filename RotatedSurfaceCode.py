...
class RotatedSurfaceCode:
    ...
    def __init__(
        ...
        self.pair_targets_round0 = [qind for anc_pos in self.even_x_ancilla_pos if anc_pos-1+1j in self.qubit_pos for qind in [self.a2i[anc_pos], self.q2i[anc_pos-1+1j]]]
        ...
        self.pair_targets_round1 = [qind for anc_pos in self.odd_x_ancilla_pos if anc_pos+1+2j in self.qubit_pos for qind in [self.a2i[anc_pos], self.q2i[anc_pos+1+2j]]]
        ...
        self.pair_targets_round2 = [qind for anc_pos in self.even_x_ancilla_pos if anc_pos-1-2j in self.qubit_pos for qind in [self.a2i[anc_pos], self.q2i[anc_pos-1-2j]]]
        ...
        self.pair_targets_round3 = [qind for anc_pos in self.odd_x_ancilla_pos if anc_pos+1-1j in self.qubit_pos for qind in [self.a2i[anc_pos], self.q2i[anc_pos+1-1j]]]
        ...
        self.stim_circuit += self.initialize_stim_circuit(gate_error_1q = noisyLogRO*gate_error_1q, gate_error_2q = noisyLogRO*gate_error_2q, measurement_error_rate = noisyLogRO*measurement_error_rate)
        self.stim_circuit += 1 * self.generate_stim_circuit_cycle(gate_error_1q = noisyLogRO*gate_error_1q, gate_error_2q = noisyLogRO*gate_error_2q, 
            idle_error_T1 = noisyLogRO*idle_error_T1, idle_error_T2 = noisyLogRO*idle_error_T2, measurement_error_rate = noisyLogRO*measurement_error_rate,detectors = False)
        ...
        self.stim_circuit += self.final_measurement_stim_circuit(gate_error_1q = noisyLogRO*gate_error_1q, gate_error_2q = noisyLogRO*gate_error_2q,measurement_error_rate = noisyLogRO*measurement_error_rate)
        ...
        if self.d <= 5:
            ...
            if boundaries:    
                for plind,pl in enumerate(z_plaquettes):
                    if len(pl) == 4:
                        # for pairs in [(pl[i],pl[(i+1)%len(pl)]) for i in range(4)]:
                        #     lattice.add_edge(pairs[0],pairs[1],e)
                        #     e += 1
                        ax.fill_between([pos[pl[0]][0],pos[pl[1]][0]],
                                        [pos[pl[3]][1],pos[pl[2]][1]],
                                        [pos[pl[0]][1],pos[pl[1]][1]],color='deepskyblue', alpha = 0.2)
                    else:
                        if self.even_z_ancilla_pos[plind].real < 3:
                        ...
                        # e += 3
                        ax.fill_between([pos[pl[1]][0],pos[ancillas[plind]][0]],
                                        [pos[pl[1]][1],pos[ancillas[plind]][1]],
                                        [pos[pl[0]][1],pos[ancillas[plind]][1]],color='deepskyblue', alpha = 0.2)
                ...
                for plind,pl in enumerate(x_plaquettes):
                    if len(pl) == 4:
                        ...
                        ax.fill_between([pos[pl[0]][0],pos[pl[1]][0]],
                                        [pos[pl[3]][1],pos[pl[2]][1]],
                                        [pos[pl[0]][1],pos[pl[1]][1]],color='red', alpha = 0.2)

                    else:
                        if self.even_x_ancilla_pos[plind].imag > 3:
                        ...
                        # lattice.add_edge(pl[1],ancillas[plind],e+2)
                        # e += 3
                        ax.fill_between([pos[pl[1]][0],pos[ancillas[plind]][0],pos[pl[0]][0]],
                                        [pos[pl[1]][1],pos[pl[1]][1],pos[pl[0]][1]],
                                        [pos[pl[0]][1],pos[ancillas[plind]][1],pos[pl[0]][1]],color='red', alpha = 0.2)
            ...
            if logicals:
                for x,y in zip([pos[i][0] for i in self.edge1_qubits],[pos[i][1] for i in self.edge1_qubits]):
                    circle1 = plt.Circle((x,y), radius=0.45, edgecolor='blue', fill= False,linewidth=1.5)
                    ax.add_patch(circle1)
                    ax.text(x-0.2,y-1.1,'Z', color='blue',fontsize=13)
                for x,y in zip([pos[i][0] for i in self.edge2_qubits],[pos[i][1] for i in self.edge2_qubits]):
                    circle1 = plt.Circle((x,y), radius=0.52, edgecolor='red', fill= False,linewidth=1.5)
                    ax.add_patch(circle1)
                    ax.text(x-1.1,y-0.2,'X', color='red',fontsize=13)
